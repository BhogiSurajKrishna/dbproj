...
module.exports = class Orders{
    ...
    static check_and_update_area_code(cur_order_id, staff_id, area_code){
        return pool.query('update customer_order set (staff_id,area_code,stat) = ($2,$3,$4) where order_no=$1 and stat = $5;', [cur_order_id, staff_id, area_code,'ready_to_dispatch','yet_to_be']);
    }

    static get_confirm_orders_staff(){
        return pool.query('select * from customer_order where stat = $1 limit 40;',['yet_to_be']);
    }
    ...
    static delivery_person_confirmed(order_id){
        return pool.query('update customer_order set stat = (case when customer_id is NULL then $2 when stat=$4 then $3 else end) where order_no=$1;', [order_id,'completed','delivered','out_for_delivery']);
    }
    ...
    static get_delivery_past_orders(userid1){
        return pool.query('select order_no,cust_name,phone, addr, bill from customer_order where stat in ($2,$3) and DP = $1;', [userid1,'completed','delivered']);
    }


    static get_user_curr_orders(id1){
        return pool.query('with id_ords as (select * from customer_order where customer_ID=$1 and not stat=$2 order by time_stamp) select delivery_person.username,delivery_person.phone as phone1,id_ords.* from id_ords left outer join delivery_person on id_ords.DP=delivery_person.ID;',[id1,'completed']);
    ...
    static get_revenue(date1,date2,a){     
        console.log(a,"1");   
        return pool.query('select date_part($1,time_stamp) as X,sum(bill) from customer_order where date(time_stamp) >= $2 and date(time_stamp) <= $3 group by X;',[a,date1,date2]);
    }

    static most_selling(date1,date2,a){
        console.log(a,"2");
        console.log('with needed(ids,times,quans,turnovers) as (select item_ID,date_part($1,order_date) as X,sum(quantity) as q1,sum(quantity*price) as p1 from food_ordered where date(order_date)>=$2 and date(order_date)<=$3 group by (item_ID,X)), tab1 as (select ids,times,quans,turnovers,ROW_NUMBER() OVER (PARTITION BY times ORDER BY turnovers desc) as rn from needed) select ids,times,quans,turnovers from tab1 where rn<=3;');
        return pool.query('with needed(ids,times,quans,turnovers) as (select item_ID,date_part($1,order_date) as X,sum(quantity) as q1,sum(quantity*price) as p1 from food_ordered where date(order_date)>=$2 and date(order_date)<=$3 group by (item_ID,X)), tab1 as (select ids,times,quans,turnovers,ROW_NUMBER() OVER (PARTITION BY times ORDER BY turnovers desc) as rn from needed) select ids,times,quans,turnovers from tab1 where rn<=3;',[a,date1,date2]);
    }
};
